...
pub fn preprocess_corpora(
    ...
    let mut processed_json: HashMap<String, HashMap<String, String>> =
        match fs::File::open(PROCESSED_JSON) {
            Ok(file) => {
                let mut reader = io::BufReader::new(file);
                serde_json::from_reader(reader)
                    .change_context(PreprocessError)
                    .attach_printable_lazy(|| format!("can't read {PROCESSED_JSON}"))?
            }
    ...
    let corpus_name = Regex::new(r"(\S+)-\d{4}-.+").unwrap();
    for zippath in fs::read_dir(RAWDIR)
        .change_context(PreprocessError)
        .attach_printable_lazy(|| format!("when opening {RAWDIR}"))?
    {
        let zippath = zippath.change_context(PreprocessError)?;
        let zippath = zippath.path();
        let zippath_name = zippath.file_name().unwrap().to_str().unwrap();
        ...
        // Get corpus name
        let prefix = if let Some(matches) = corpus_name.captures(zippath_name) {
        ...
        let corpus_source_dir = Path::new("material")
            .join(corpus_id)
            .join("source")
            .join(Path::new(zippath.file_stem().unwrap()).file_stem().unwrap());
        make_corpus_config(
            corpus_id,
            name,
            descr,
        ...
        let zip_file = fs::File::open(zippath).unwrap();
        let mut zipf = ZipArchive::new(zip_file).change_context(PreprocessError)?;
        //         for zipobj in zipf.filelist:
        for i in 0..zipf.len() {
            let zipobj = zipf.by_index(i).unwrap();
...
impl fmt::Display for PreprocessError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("Error preprocessing")
    }
}

impl Context for PreprocessError {}

// pub fn preprocess_xml(xml_string, filename, testfile=False):
//     """ -> error_stack::Result<(x, PreprocessError> {ract meta data and html from f."""
...
// pub fn process_html(elem, textelem, filename, testfile=False):
//     " -> error_stack::Result<(P, PreprocessError> {ocess the actual text content of the document."""
//  pub fn clean_html(text):
//         # Remove chars between 0-31 and 127 -> error_stack::Result<(1, PreprocessError> {9, but keep 10 (line break).
...
pub fn make_corpus_config(corpus_id: &str, name: &str, descr: &str, mut path: PathBuf) {
    //     """Writ -> error_stack::Result<(), PreprocessError> { Sparv corpus config file for sub corpus."""
    ...
    writer.write("parent: ../config.yaml\n".as_bytes()).unwrap();
    writer.write("\n".as_bytes()).unwrap();
...
// pub fn write_xml(text, xmlpath):
//     """Wrap 'text' in a file tag an -> error_stack::Result<(), PreprocessError> { save as 'xmlpath'."""
...
// pub fn write_json(data, filepath):
//     """Write json data to filepat -> error_stack::Result<(), PreprocessError> {."""
